...
clusterFunctionInputs <- function(id, label = "inputs",isRSEC=FALSE) {
    ...
    ns <- NS(id)
    alphaInput<-vectorInput(id,"Set alpha?", "e.g. 0.1,0.2,0.3",val="alphas",  help="List of comma-separated values between 0 and 1 giving values of alpha to be used by 0-1 clustering functions. Determines tightness required in creating clusters from the dissimilarity matrix.",required=FALSE, functionName="clusterMany")
